...
export function NotionRevenueSettings() {
  const utils = trpc.useUtils();
  const { data: connection } = trpc.revenue.connection.get.useQuery();

  const [databaseId, setDatabaseId] = useState(connection?.revenueDatabaseId ?? '');
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="revenue-database-id">Database ID or URL</Label>
          <Input
            id="revenue-database-id"
